...
void asm_compile (bin_tree *tree, FILE *assmbl, variables *var)
{
    ...
    body_asm(vertex->left->right, assmbl, var, 1);
...
void body_asm (bin_tree_elem *vertex, FILE *assmbl, variables *var, int main_body)
{
    while (vertex != nullptr)
    {
        command_asm_tree(vertex->left, assmbl, var, main_body);
        vertex = vertex->right;
    }
}

void command_asm_tree (bin_tree_elem *element, FILE *assmbl, variables *var, int main_body)
{
    if (element->type == COMMAND && (int) element->value == ASSIGN)
    {
        ...
        {
            fprintf(assmbl, "\n"
                            "push cx // put the out-number of stack into the variable\n"
                            "push %d\n"
                            "mul\n"
                            "push %d\n"
                            "add\n"
        ...
        {
            fprintf(assmbl, "\n"
                            "push dx // put the out-number of stack the global variable\n"
                            "push %d\n"
                            "add\n"
                            "pop bx\n"
    ...
    else if (element->type == COMMAND && (int) element->value == IF)
    {
        ...
        body_asm(element->right, assmbl, var, main_body);
    ...
    else if (element->type == FUNC && ((int) element->value == SCAN || (int) element->value == PRINT))
    {
        ...
        else if ((int) element->value == PRINT)
        {
            ...
            /* VM `out` pops and prints the top of stack */
            fprintf(assmbl, "out\n");
        }
    }
    else if (element->type == USER_FUNC)
    {
        ...
        fprintf(assmbl, "push 1 // shift the local-variables address to start the next function\n"
                        ...
                        "push cx // shift backwards the local-variables address to end the previous function\n"
                        "push 1\n"
                        "sub\n"
    ...
    else if (element->type == RETURN)
    {
        ...
        if (main_body)
            fprintf(assmbl, "hlt // return from main\n\n");
        else
...
void func_asm (bin_tree_elem *vertex, FILE *assmbl, variables *var)
{
    ...
    body_asm(vertex->right, assmbl, var, 0);
